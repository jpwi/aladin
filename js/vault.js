...
const Vault = {
    ...
    async saveToFile(data) {
        ...
        try {
            ...
            // If createWritable fails (e.g., file locked by cloud sync),
            // try alternative approach with retry
            if (error.name === "NoModificationAllowedError" || 
                error.message.includes("locked") ||
                error.message.includes("in use")) {
                console.warn(
                    "Vault: File may be locked by cloud sync, retrying...",
                );
                // Wait a moment for cloud sync to release lock
